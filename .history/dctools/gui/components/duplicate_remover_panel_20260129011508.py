...
class DuplicateRemoverPanel(ttk.LabelFrame):
    """文件去重面板"""
    ...
    def __init__(self, parent, main_app):
        ...
        """
        super().__init__(parent, text="🗑️ 文件去重", padding=20)
    ...
    def _create_widgets(self):
        ...
        ttk.Label(method_frame, text="去重方式:", font=("Microsoft YaHei", 10)).pack(side=tk.LEFT, padx=5)
        self.method_var = tk.StringVar(value="hash")
        method_options = ["name", "size", "mtime", "hash"]
        ttk.Combobox(method_frame, textvariable=self.method_var, values=method_options, width=10).pack(side=tk.LEFT, padx=5)
    ...
    def _preview_duplicates(self):
        ...
        try:
            from dctools.core.duplicate_remover import preview_duplicates
            # 重定向输出到预览文本框
            import io
            import sys
            ...
            try:
                removed_count = preview_duplicates(folder_path, method)
            finally:
    ...
    def _remove_duplicates(self):
        ...
        try:
            from dctools.core.duplicate_remover import remove_duplicates
