...
class FolderInfoPanel(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        output_frame = ttk.LabelFrame(self, text="导出格式", padding=10)
        ...
        self.result_text = tk.Text(result_frame, height=8, width=60, font=('Courier New', 9),
    ...
    def _setup_drag_drop(self):
        ...
        try:
            ...
            if hasattr(root, 'drop_target_register'):
                # 注册拖放目标
                root.drop_target_register('DROPFILES')
                root.dnd_bind('<<Drop>>', self._on_drop)

                # 也为当前面板注册
                if hasattr(self, 'drop_target_register'):
                    self.drop_target_register('DROPFILES')
                    self.dnd_bind('<<Drop>>', self._on_drop)

                print("拖放功能启用成功")
            else:
                print("当前Tkinter版本不支持标准拖放功能")

        except Exception as e:
            # 拖放功能不支持时静默失败
            print(f"拖放功能初始化失败: {str(e)}")
            pass
