...
def get_image_properties(image_path):
    ...
    try:
        ...
        logger.warning(f"获取图片属性失败 {image_path}: {str(e)}")
        return {}


class MediaPropertyHandler:
    """媒体属性处理器基类"""
    def remove_properties(self, media_path, properties_to_remove=None, remove_all=False):
        """移除媒体属性"""
        pass

    def add_property(self, media_path, property_name, property_value):
        """添加媒体属性"""
        pass

class ImagePropertyHandler(MediaPropertyHandler):
    """图片属性处理器"""
    def remove_properties(self, media_path, properties_to_remove=None, remove_all=False):
        try:
            if remove_all:
                ...
                logger.info(f"成功移除图片属性: {media_path}")
                return True
            else:
                # 移除指定属性
                if properties_to_remove:
                    ...
                    if exif_dict:
                        ...
                        logger.warning(f"无法加载图片EXIF数据: {media_path}")
                        return False
                else:
                    logger.warning(f"未指定要移除的属性: {media_path}")
                    return False
        except Exception as e:
            logger.warning(f"处理图片文件失败 {media_path}: {str(e)}")
            return False
            ...
            with Image.open(media_path) as img:
                ...
                if tag_id:
                    ...
                    try:
                        ...
                        # 保存图片
                        img.save(media_path, exif=exif_bytes)
                        logger.info(f"成功添加图片属性 {property_name}: {media_path}")
                        return True
                    except Exception as e:
                        logger.warning(f"使用piexif添加图片属性失败 {media_path}: {str(e)}")
                        # 回退到PIL的方法
                        try:
                            exif_data = img.getexif()
                            exif_data[tag_id] = property_value
                            img.save(media_path, exif=exif_data.tobytes())
                            logger.info(f"成功添加图片属性 {media_path}")
                            return True
                        except Exception as e2:
                            logger.warning(f"添加图片属性失败 {media_path}: {str(e2)}")
                            return False
                else:
                    logger.warning(f"未知的属性名称: {property_name}")
                    return False
        except Exception as e:
            logger.warning(f"处理图片文件失败 {media_path}: {str(e)}")
            return False

class AudioPropertyHandler(MediaPropertyHandler):
    """音频属性处理器"""
    def remove_properties(self, media_path, properties_to_remove=None, remove_all=False):
        try:
            # 使用mutagen库移除ID3标签
            ...
            if audio:
                if remove_all:
                    ...
                    # 保存更改
                    audio.save()
                    logger.info(f"成功移除音频属性: {media_path}")
                    return True
                else:
                    # 移除指定标签
                    if properties_to_remove:
                ...
                try:
                    ...
                    if remove_all:
                        ...
                        if properties_to_remove:
                            ...
                            for prop in properties_to_remove:
                                tag_name = id3_tag_mapping.get(prop, prop.upper())
                    ...
                    logger.warning(f"处理音频文件失败 {media_path}: {str(e2)}")
                    return False
        except Exception as e:
            logger.warning(f"处理音频文件失败 {media_path}: {str(e)}")
            return False
    ...
    def add_property(self, media_path, property_name, property_value):
        try:
            ...
            if audio:
                ...
                audio.save()
                logger.info(f"成功添加音频属性 {property_name}: {media_path}")
                return True
            else:
                # 尝试使用ID3直接处理
                try:
                    ...
                    if property_name in id3_tag_mapping:
                        ...
                        logger.warning(f"不支持的音频属性: {property_name}")
                        return False
                except Exception as e2:
                    logger.warning(f"处理音频文件失败 {media_path}: {str(e2)}")
                    return False
...
class VideoPropertyHandler(MediaPropertyHandler):
    """视频属性处理器"""
    def remove_properties(self, media_path, properties_to_remove=None, remove_all=False):
        try:
            # 首先尝试不使用easy模式
            ...
            if video:
                    ...
                    if properties_to_remove and hasattr(video, 'tags') and video.tags is not None:
                        ...
                        for prop in properties_to_remove:
                            ...
                            # 也尝试直接使用原始属性名
                                ...
                                del video.tags[prop]
                video.save()
                logger.info(f"成功移除视频属性: {media_path}")
                return True
            else:
                # 尝试使用easy模式
                ...
                if video_easy:
                    ...
                    logger.warning(f"视频文件不支持属性编辑: {media_path}")
                    return False
        except Exception as e:
            logger.warning(f"处理视频文件失败 {media_path}: {str(e)}")
            return False
            ...
            if video:
                ...
                try:
                    ...
                    if hasattr(video, 'tags') and video.tags is not None:
                        ...
                        video.save()
                        logger.info(f"成功添加视频属性 {property_name}: {media_path}")
                        return True
                    else:
                        # 尝试使用easy模式
                        video_easy = mutagen.File(media_path, easy=True)
                    ...
                    logger.warning(f"处理视频文件失败 {media_path}: {str(e2)}")
                    return False
            else:
                logger.warning(f"视频文件不支持属性编辑: {media_path}")
                return False
...
class MediaPropertyHandlerFactory:
    """媒体属性处理器工厂"""
    @staticmethod
    def get_handler(media_path):
        """根据文件扩展名获取对应的处理器"""
        ext = os.path.splitext(media_path)[1].lower()

        # 图片文件
        if ext in ['.jpg', '.jpeg', '.png', '.bmp', '.webp']:
            return ImagePropertyHandler()
        # 音频文件
        elif ext in ['.mp3', '.wav', '.flac', '.aac']:
            return AudioPropertyHandler()
        # 视频文件
        elif ext in ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv']:
            return VideoPropertyHandler()
        # 不支持的文件类型
        else:
            return None
...
def remove_media_properties(media_path, properties_to_remove=None, remove_all=False):
    ...
    try:
        # 获取对应的处理器
        handler = MediaPropertyHandlerFactory.get_handler(media_path)
        if handler:
            return handler.remove_properties(media_path, properties_to_remove, remove_all)
        else:
            logger.warning(f"不支持的文件类型: {media_path}")
            return False
    except Exception as e:
        logger.error(f"移除媒体属性失败 {media_path}: {str(e)}")
        return False
...
def add_media_property(media_path, property_name, property_value):
    ...
    try:
        ...
        handler = MediaPropertyHandlerFactory.get_handler(media_path)
        if handler:
            return handler.add_property(media_path, property_name, property_value)
        else:
            logger.warning(f"不支持的文件类型: {media_path}")
            return False
    except Exception as e:
        logger.error(f"添加媒体属性失败 {media_path}: {str(e)}")
        return False
...
def batch_remove_properties(folder_path, properties_to_remove=None, remove_all=False, recursive=True):
    ...
    logger.info(f"开始批量移除媒体属性: {folder_path}")
    logger.info(f"递归: {'是' if recursive else '否'}")
    logger.info(f"移除所有属性: {'是' if remove_all else '否'}")
    ...
    logger.info("批量移除媒体属性完成！")
    logger.info(f"成功: {processed_count} 个文件")
...
def batch_add_property(folder_path, property_name, property_value, recursive=True):
    ...
    logger.info(f"开始批量添加媒体属性: {folder_path}")
    logger.info(f"属性: {property_name} = {property_value}")
    logger.info(f"递归: {'是' if recursive else '否'}")
